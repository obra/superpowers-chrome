...
var UseBrowserSchema = external_exports.object({
  action: external_exports.nativeEnum(BrowserAction).describe("Browser action to perform"),
  tab_index: external_exports.number().int().min(0).default(0).describe("Tab index to operate on (default: 0)"),
  selector: external_exports.string().nullable().default(null).describe("CSS selector for element operations (required for click, type, select, attr, await_element)"),
  payload: external_exports.union([external_exports.string(), external_exports.array(external_exports.string())]).nullable().default(null).describe("Action payload: URL for navigate, text for type (append \\n to submit), format for extract, filename for screenshot, code for eval, value(s) for select, attribute name for attr, text for await_text"),
  timeout: external_exports.number().int().min(0).max(6e4).default(5e3).describe("Timeout in milliseconds for await operations (default: 5000)")
});
